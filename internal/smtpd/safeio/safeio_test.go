...
func TestReadLine_Logging(t *testing.T) {
	...
	expected := "=> DATA\r\n"
	if got := logger.String(); !strings.Contains(got, expected) {
...
func TestLoggingToggle(t *testing.T) {
	...
	if got := logger.String(); len(got) > 0 {
		t.Errorf("logging disabled but log written: %q", got)
	}
	...
	if got := logger.String(); !strings.Contains(got, "<= 250 Reset\r\n") {
		t.Errorf("logging re-enabled but not working, got = %q", got)
	}
}
